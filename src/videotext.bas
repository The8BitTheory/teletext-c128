...
5 ML=DEC("2800"):BE=PEEK(4624)+PEEK(4625)*256
...
387 S=TI-S
389 PRINT "COPY TO VRAM TOOK "S" JIFFIES"
...
393 S=TI:GOSUB 1010:S=TI-S:PRINT "SUB TOOK "S"JIFFIES"
...
480 IF I$="+" THEN GOSUB 1200
481 IF I$="-" THEN GOSUB 1300
...
1010 SYS ML,ASC(I$(0)),ASC(I$(1)),ASC(I$(2)),,ASC(S$(0)),ASC(S$(1))
...
1150 V=PEEK(C1+1)
1160 IF V>=ASC("0") AND V<=ASC("9") THEN SM=VAL(CHR$(V)):RETURN

1170 SM=VAL(CHR$(PEEK(C1))+CHR$(PEEK(C1+1)))
...
####################
1200 SX=SX+1
1210 IF SX>SM THEN SX=1

1220 V$=MID$(STR$(SX),2):PRINT "SUB:"V$
1230 IF SX>9 THEN S$(0)=LEFT$(V$,1):S$(1)=RIGHT$(V$,1):RETURN
# IF SX<=9
1240 S$(0)="0":S$(1)=V$
1299 RETURN
