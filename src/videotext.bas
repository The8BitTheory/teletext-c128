...
12 ML=DEC("3880"):BE=PEEK(4624)+PEEK(4625)*256
14 IF ML<=BE THEN PRINT "BASIC PROGRAM TOO LARGE. ENDS AT $"HEX$(BE):END

# SUPERCPU128 DETECTION
20 SC=ABS((PEEK(53433)AND128)=128)
22 IF SC=0 THEN SC=-1:PRINT "SUPERCPU DETECTED. DISABLING.":ELSE SC=0
# DISABLE TURBO MODE
24 IF SC THEN POKE 53370,0


50 PRINT"LOADING VDCBASIC...";:BLOAD"VDCBASIC2G.1300",U(DD),B0:PRINT"DONE"
52 SYS DEC("1300")
...
56 SF=RGD(255)
58 IF SF=0 THEN PRINT "64KB VRAM BENOETIGT, SORRY.":END

# GET NR OF SCREEN ROWS. 32 MEANS NTSC, 39 MEANS PAL
60 TN=RGD(4):SO=25+90*80:AO=25+10*80
...
62 RGO 28,16:REM 64K VRAM
64 RGO 25,128:REM BITMAP MODE ON
...
66 IF TN>35 THEN RGW4,35:RGW5,5
68 IF TN=32 THEN RGW4,29:RGW5,3
...
110 PRINT "LOADING CHARSET... ";:BLOAD"TXTCHARSET3.VDC",U(DD),B0,P(BE+5120)
111 PRINT "DONE"
120 FL=PEEK(174)+PEEK(175)*256-BE
130 RTV BE+5120,56*1024,FL
140 VCS 56*1024,2,9,80

200 PRINT "LOADING ML ROUTINES... ";:BLOAD"C128TEXT.BIN",U(DD),B0
201 PRINT "DONE"
...
#225 IF DB THEN PRINT "LOCAL TIME AT $"HEX$(LT)

#   READ ADDRESS OF HTML CONTENT
230 C0=PEEK(DEC("FD"))+PEEK(DEC("FE"))*256
#235 IF DB THEN PRINT:PRINT "CONTENT AT $"HEX$(C0)

# RV:RETURN-VALUE FROM WIC64 INITIALIZATION (0=OK, ALL OTHERS ARE ERROR CODES)
240 IF RV<>0 THEN GOSUB 2000

# ENABLE SUPERCPU TURBO MODE
#250 IF SC THEN PRINT "ENABLING SCPU TURBO...";:POKE 53371,0:PRINT"DONE"

250 GOSUB 2600
259 WINDOW 21,1,39,23,1

#   REQUEST PAGE 100
300 VMP S(R)+20,"HOLE SEITE                    "
301 VMF A(R)+20,DEC("F0"),60
302 IF F THEN FAST
...
#RL:RESULT LENGTH FROM WIC64 REQUEST. 2100 SHOWS ERROR PAGE, 2400 NOT FOUND
310 IF RL=0 THEN 2400
...
360 S=TI-S
#365 IF DB THEN PRINT "COPY TO VRAM TOOK "S" JIFFIES"
...
392 IF OM>0 THEN VMP S(R)+46,"OFFLINE MODUS":GOTO 396

#   WRITE LOCAL TIME
393 SYS ML+12
394 VMB S(R)+42,LT,19

395 IF IK THEN VMP S(ABS(R-1))+18*720+320+32,"- TASTE -":GETKEY I$:IK=0
...
450 IF I$="X" THEN PRINT "":SLOW:END
# LEFT: 157
452 IF I=157 THEN PD=-1:GOTO 2500
# RIGHT: 29
454 IF I=29 THEN PD=1:GOTO 2500

460 IF I$="R" THEN 300
# HOME - STARTSEITE
465 IF I=19 THEN I$(0)="1":I$(1)="0":I$(2)="0":SX=1:SM=1:GOSUB1320:GOTO 300
470 IF I$="F" THEN 2700
475 IF I$="M" THEN 2800
480 IF I$="+" THEN 1300
481 IF I$="-" THEN 1200
...
900 FL$=FL$+".HTML"
905 PRINT "SAVING "FL$" ...";
#909 IF SC THEN POKE 53370,0
910 BSAVE ("@"+FL$),U(DD),B0,P(RS) TO P(RE)
#911 IF SC THEN POKE 53371,0
920 PRINT "DONE"
930 RETURN
...
950 FL$=FL$+".VTX"
955 PRINT "SAVING "FL$" ...";
#959 IF SC THEN POKE 53370,0
960 BSAVE ("@"+FL$),U(DD),B0,P(DA) TO P(DA+2024)
#961 IF SC THEN POKE 53371,0
970 PRINT "DONE"
980 RETURN
...
1020 S=TI-S
#1021 IF DB THEN PRINT "DL TOOK "S" JIFFIES"
...
#1070 IF DB THEN PRINT "RS="RS",RE="RE",RL="RL
...
#1084 IF SC THEN POKE 53370,0
1085 BLOAD (FL$+".VTX"),U(DD),B0,P(C0)
#1086 IF SC THEN POKE 53371,0
...
# LI:LOOP TIMESTAMP
1400 LM=-1
1420 LP(0)=401:LP(1)=409:LP(2)=LP(0):LP(3)=1
...
# CREATE QR-CODE
# QL:QR-CODE LOCATION IN MEMORY
# QS:QR-CODE SIZE OF ONE AXIS (EG 29)
1725 S=TI:SYS ML+9:S=TI-S:RREGRA,RX,RY
1730 QL=RX+RY*256:QS=RA
1735 UL=PEEK(DEC("FB"))+PEEK(DEC("FC"))*256

#1740 IF DB THEN PRINT "CREATE QR TOOK "S" JIFFIES"
...
# PROBLEM PAGE #
################
2100 IF IK THEN VMP S(ABS(R-1))+18*720+320+32,"- TASTE -":GETKEY I$:IK=0
2105 GOSUB 500
2110 DISP S(R):ATTR A(R)
2115 VMP S(R)+1440,"SEITE KONNTE NICHT GELADEN WERDEN"
...
2190 IF I$="X" THEN SLOW:PRINT "":END
...
# TITLE SCREEN #
################
2300 VMF A(R)+80,DEC("F0"),1840
2302 VMF S(R)+720,255,18000-1440

2305 VMP S(R)+720*3+30,"GOODWELL'S"
2310 VMP S(R)+720*5+20,"ARD VIDEOTEXT VIEWER"
2312 VMP S(R)+720*6+23,"F"+CHR$(130)+"R WIC64 UND VDC"

2315 SYS ML+18:RREGRA,RX,RY
2320 QL=RX+RY*256:QS=RA


# WRITE URL
2345 VMP S(R)+16*720+2,"GITHUB.COM/THE8BITTHEORY/TELETEXT-C128"
2350 VMP S(R)+18*720+320+36,"LADE"

# COPY QR CODE FROM RAM TO VRAM
2355 RTV QL,40000,QS*QS

# COPY QR-CODE TO SCREENBUFFER
2360 VMC 40000,S(R)+SO-1440+1+160,QS,QS,160
2365 VMC 40000,S(R)+SO-1440+1+80+160,QS,QS,160
...
# PAGE NOT FOUND #
##################
# THIS IS AN ALTERNATIVE TO THE PROBLEM PAGE AT 2100
# WE JUST WRITE A "NOT FOUND" INFO ON TOP AND KEEP THE CURRENT PAGE

2400 R=ABS(R-1)
2405 VMP S(R)+20,"     SEITE "+I$(0)+I$(1)+I$(2)+" NICHT GEFUNDEN "
2410 VMF A(R)+20,DEC("90"),60
2415 VMF A(R),DEC("F0"),6
...
# PAGE NEXT/PREV #
##################
# INCREASE/DECREASE PAGE, NOT SUB-PAGE
# NP:NEXT PAGE
# PD:PAGE-DELTA (+1 OR -1)
# SX:SUBPAGE INDEX (1-99)
2500 NP=VAL(I$(0)+I$(1)+I$(2))
2505 NP=NP+PD
2510 IF NP<100 THEN NP=899
2515 IF NP>899 THEN NP=100
2520 NP$=MID$(STR$(NP),2)
2530 FOR X=0TO2
2540  I$(X)=MID$(NP$,X+1,1)
2550 NEXT
2555 S$(0)="0":S$(1)="1":SX=1
...
#############
2600 PRINT ""
2602 PRINT "  ARD-TEXT VIEWER"
2604 PRINT "   C128,VDC,64K":PRINT
2606 PRINT "  2025 BY GOODWELL":PRINT
2608 PRINT ""
2610 PRINT "  0-9 :SEITE ANW. "
2615 PRINT " HOME :ZU S.100   "
2620 PRINT "   M  :MENUE S.790"
2625 PRINT "RECHTS:NAECHSTE S."
2630 PRINT "LINKS :VORIGE S.  "
2635 PRINT "   +  :SUBSEITE + "
2640 PRINT "   -  :SUBSEITE - "
2645 PRINT "   R  :NEU LADEN  "
2650 PRINT "   F  :FAST/SLOW  "
2655 PRINT "   L  :DEMO MODUS "
2660 PRINT "   Q  :QR-CODE    "
2665 PRINT "   D  :SPEICHERN  "  
2670 PRINT "   O  :OFFLINE    "
2675 PRINT "   X  :EXIT       "
2680 PRINT ""
...
# FAST MODE
2700 F=ABS(F)-1
2710 PRINT "FAST MODE:"OM$(ABS(F)):GOTO 400

# GO TO MENU
2800 I$(0)="7":I$(1)="9":I$(2)="0":SX=1:SM=1:GOSUB1320:GOTO 300
...
60000 SLOW
60010 IF ER=4 THEN 61000
60020 PRINT ""ERR$(ER)
60099 END

# DRIVE ERRORS
61000 IF DS=74 THEN PRINT:PRINT "LAUFWERK NICHT BEREIT.":PRINT"BITTE DISK EINLEGEN.":GOTO 61080
